...
export type NodeFilterMap = Record<NodeFilterMapKey, JSONQueryField[]> &
  QueryMap;
...
export function isJoinMap(x: unknown): x is Record<string, JoinMap[]> {
  return isPlainObject(x) && Object.keys(x).every(v => isString(v));
}
...
export function isJSONQuery(data: unknown): data is JSONQuery {
  ...
  if (!('join' in data.result_query) || !('fields' in data.result_query))
    return false;
